...
/* global Chart, location, document, port, socketPath, io, HTMLCanvasElement, span */
 ...
 *
 * @param {HTMLCanvasElement} ctx - The canvas element to render the chart in
 * @param {Array} dataset - An array of datasets to display on the chart
...
/**
 * Extracts the timestamp from a given data point.
 *
 * @param {object} point - The data point object containing a timestamp.
 * @returns {number|string} The timestamp associated with the data point.
 */
...
/**
 * Called when the user changes the time span for the charts
 *
...
socket.on('esm_start', function (data) {
  ...
  eventLoopChart.data.datasets[0].data = data[defaultSpan].os.map(
    function (point) {
      if (point.loop) {
...
socket.on('esm_stats', function (data) {
  ...
  if (
    ...
    if (os) {
      heapStat.textContent =
        (os.heap.used_heap_size / 1024 / 1024).toFixed(1) + 'MB';
      heapChart.data.datasets[0].data.push(
        os.heap.used_heap_size / 1024 / 1024
      );
